...
namespace Cheetah.LaserTools
{
    static class Controls
    {
        public static IMyTerminalControlSlider LaserBeam<T>() where T: IMyTerminalBlock
        {
            ...
            LaserBeam.SetLimits(Block => BlockReturn(Block, x => x.BeamCtlModule.MinBeamLengthBlocks), Block => BlockReturn(Block, x => x.BeamCtlModule.MaxBeamLengthBlocks));
            LaserBeam.Getter = (Block) => BlockReturn(Block, x => x.TermModule.BeamLength);
            LaserBeam.Setter = (Block, NewLength) => BlockAction(Block, x => x.TermModule.BeamLength = (int)NewLength);
            LaserBeam.Writer = (Block, Info) => Info.Append($"{BlockReturn(Block, x => x.TermModule.BeamLength)} blocks");
            return LaserBeam;
        }
        ...
        public static IMyTerminalControlSlider SpeedMultiplier<T>() where T : IMyTerminalBlock
        {
            ...
            SpeedMultiplier.Getter = (Block) => BlockReturn(Block, x => x.TermModule.SpeedMultiplier);
            SpeedMultiplier.Setter = (Block, NewSpeed) => BlockAction(Block, x => x.TermModule.SpeedMultiplier = (int)NewSpeed);
            SpeedMultiplier.Writer = (Block, Info) => Info.Append($"x{BlockReturn(Block, x => x.TermModule.SpeedMultiplier)}");
        ...
        public static IMyTerminalControlCheckbox DistanceMode<T>() where T : IMyTerminalBlock
        {
            ...
            DistanceMode.Getter = (Block) => BlockReturn(Block, x => x.TermModule.DistanceMode);
            DistanceMode.Setter = (Block, NewMode) => BlockAction(Block, x => x.TermModule.DistanceMode = NewMode);
            return DistanceMode;
        }
